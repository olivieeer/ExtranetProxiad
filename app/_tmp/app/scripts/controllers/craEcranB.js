...
var craEcranBController = angular.module('craEcranBController', []);

craEcranBController.controller('CraEcranBCtrl', ['$scope',
  function ($scope) {
    ...
    $scope.headerCraToToggle = function(nb) {
      if (!nb.hasOnClick) {
        return 0;
      }
      if (!nb.hasBeenToggle) {
		...
		$scope.lignesCra[0][nb.label+2].class = "tdDayNotDisabled";
      ...
      for (k = 1; k <= nbDaysInMonth; k++) {
        ...
        if (dayOfWeek > 0 && dayOfWeek < 6) {
          ...
          if ( sumQuotidienneTravaillee + sumQuotidienneAbsence != 1) {
            ...
            if ( typeof $scope.lignesCra[0][k+2] === 'object') {
              $scope.lignesCra[0][k+2].class ="tdInError";
            }
